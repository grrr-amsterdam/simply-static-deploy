...
const Deployer = ($) => {
  ...
  const showError = (message) => {
    ...
    $statusList
      .find('span[class="is-current"]')
      .html(createTaskStatusTemplate("failed"));
  };
  ...
  const getPreviousTask = () =>
    PREVIOUSLY_QUEUED_TASKS.length
  ...
  const performQueuedTasks = (isSingle = false) => {
    ...
    if (!next & !isSingle) {
      $deployTime.html(
        `Just deployed, <a href="${VARS.website}" target="_blank">view site</a>.`
      );
    }
    if (!next) {
      enableTriggerButtons();
